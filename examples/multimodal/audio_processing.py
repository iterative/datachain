...
def main():
    ...
    audio_chain = dc.read_storage(audio_source).filter(
        C("file.path").glob("*.wav")
    ...
    if total_files == 0:
        print(
            "No audio files found. Please add some audio files to the 'audio_files/' directory."
        )
    ...
    metadata_chain = audio_chain.map(
        metadata=extract_audio_metadata, output=AudioMetadataFeature
    )
    ...
    features_chain = valid_audio_chain.map(
        features=extract_audio_features, output=AudioFeaturesFeature
    )
    ...
    if processed_chain.count() > 0:
        ...
        print(
            "- Average duration:",
            processed_chain.select("metadata.duration")
            .to_pandas()["metadata.duration"]
            .mean(),
        )
        print(
            "- Average RMS energy:",
            processed_chain.select("features.rms_energy")
            .to_pandas()["features.rms_energy"]
            .mean(),
        )
...
def create_sample_data():
    ...
    if not os.path.exists(sample_dir):
        ...
        print(
            "Please add some audio files (.wav, .mp3, .flac) to this directory to run the example."
        )
