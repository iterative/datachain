...
def main():
    ...
    analysis_chain = (
        chain.map(stats=extract_audio_statistics).filter(
            lambda stats: stats["rms"] > 0.01
        )  # Filter out very quiet segments
    )
    ...
    for file, meta in chain.to_iter("file", "meta"):
        ...
        for fragment in file.get_fragments(duration=5.0, audio_duration=meta.duration):
            ...
            memory_mb = estimate_memory_usage(
                fragment.end - fragment.start, meta.sample_rate, meta.channels
            )
            print(
                f"  Fragment {fragment.start:.1f}s-{fragment.end:.1f}s: ~{memory_mb:.1f}MB"
            )
    ...
    for file, meta in chain.to_iter("file", "meta"):
        ...
        for i in range(3):
            ...
            if end > start:
                ...
                print(
                    f"  Fragment {start:.1f}s-{end:.1f}s: {quality} quality (RMS: {stats['rms']:.4f})"
                )
