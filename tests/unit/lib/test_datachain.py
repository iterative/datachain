...
def test_apply_pattern_filtering_helper(test_session):
    ...
    filtered_chain = _apply_pattern_filtering(chain, ["cat*", "description"], "file_names")
    assert filtered_chain.count() == 3
    assert set(filtered_chain.to_values("file_names.path")) == {"cat1", "mydir/cat2", "description"}
