...
class TestSplitUriPattern:
    ...
    def test_globstar_pattern(self):
        ...
        assert split_uri_pattern("s3://bucket/dir/**/*.txt") == ("s3://bucket/dir", "**/*.txt")
        assert split_uri_pattern("file:///data/**/test/*.json") == ("file:///data", "**/test/*.json")
    
    def test_brace_expansion(self):
        """Test brace expansion patterns like *.{mp3,wav}"""
        assert split_uri_pattern("s3://bucket/*.{mp3,wav}") == ("s3://bucket", "*.{mp3,wav}")
        assert split_uri_pattern("s3://bucket/dir/*.{jpg,png,gif}") == ("s3://bucket/dir", "*.{jpg,png,gif}")
        assert split_uri_pattern("file:///data/**/*.{json,jsonl}") == ("file:///data", "**/*.{json,jsonl}")
    
    def test_question_mark(self):
        """Test question mark wildcards like file?.txt"""
        assert split_uri_pattern("s3://bucket/file?.txt") == ("s3://bucket", "file?.txt")
        assert split_uri_pattern("s3://bucket/dir/doc??.pdf") == ("s3://bucket/dir", "doc??.pdf")
        assert split_uri_pattern("file:///data/test?/file.txt") == ("file:///data", "test?/file.txt")
    
    def test_combined_patterns(self):
        """Test combinations of different glob patterns"""
        assert split_uri_pattern("s3://bucket/**/test?.{mp3,wav}") == ("s3://bucket", "**/test?.{mp3,wav}")
        assert split_uri_pattern("s3://bucket/*/dir/**/*.txt") == ("s3://bucket", "*/dir/**/*.txt")
    ...
    def test_edge_cases(self):
        ...
        assert split_uri_pattern("s3://bucket/dir/subdir/*.mp3") == ("s3://bucket/dir/subdir", "*.mp3")
...
class TestReadStorageGlobPatterns:
    """Test read_storage with glob pattern support"""
    ...
    def test_wildcard_pattern(self, mock_read_dataset, mock_ls, mock_get_listing, mock_session, mock_listing):
        ...
        result = dc.read_storage(f"{tmp_dir.as_uri()}/audio/*.mp3", session=mock_session)
    ...
    @patch("datachain.lib.dc.storage.ls")
    @patch("datachain.lib.dc.datasets.read_dataset")
    def test_globstar_pattern(self, mock_read_dataset, mock_ls, mock_get_listing, mock_session, mock_listing):
    ...
    @patch("datachain.lib.dc.storage.ls")
    @patch("datachain.lib.dc.datasets.read_dataset")
    def test_brace_expansion_pattern(self, mock_read_dataset, mock_ls, mock_get_listing, mock_session, mock_listing):
        """Test that brace expansion patterns work correctly"""
        tmp_dir, files = mock_listing
        
        # Setup mocks
        mock_get_listing.return_value = ("test_dataset", str(tmp_dir), "audio/*.{mp3,wav}", True)
        mock_chain = MagicMock()
        mock_query = MagicMock()
        mock_chain._query = mock_query
        mock_chain.signals_schema = MagicMock()
        ...
        result = dc.read_storage(f"{tmp_dir.as_uri()}/audio/*.{{mp3,wav}}", session=mock_session)
    ...
    @patch("datachain.lib.dc.storage.ls")
    @patch("datachain.lib.dc.datasets.read_dataset")
    def test_question_mark_pattern(self, mock_read_dataset, mock_ls, mock_get_listing, mock_session, mock_listing):
        """Test that question mark wildcards work"""
        tmp_dir, files = mock_listing
        
        # Setup mocks
        mock_get_listing.return_value = ("test_dataset", str(tmp_dir), "file?.txt", True)
        mock_chain = MagicMock()
        mock_query = MagicMock()
        mock_chain._query = mock_query
        mock_chain.signals_schema = MagicMock()
    ...
    def test_multiple_patterns(self, mock_read_dataset, mock_ls, mock_get_listing, mock_session, mock_listing):
        ...
        mock_chain1.signals_schema.mutate = MagicMock(return_value=mock_chain1.signals_schema)
        mock_chain2.signals_schema.mutate = MagicMock(return_value=mock_chain2.signals_schema)
        ...
        uris = [
            f"{tmp_dir.as_uri()}/audio/*.mp3",
            f"{tmp_dir.as_uri()}/docs/*.json"
        ]
    ...
    @patch("datachain.lib.dc.storage.ls")
    @patch("datachain.lib.dc.datasets.read_dataset")
    def test_no_pattern_unchanged(self, mock_read_dataset, mock_ls, mock_get_listing, mock_session, mock_listing):
...
class TestGlobPatternExpansion:
    ...
    def test_expand_brace_to_glob(self):
