...
def test_hf():
    ...
    gen = HFGenerator(ds, dict_to_data_model("", schema, original_names=list(ds.features.keys())))
    gen.setup()
...
def test_hf_split():
    ...
    gen = HFGenerator(ds_dict, dict_to_data_model("", schema, original_names=["split"] + list(ds_dict["train"].features.keys())))
    gen.setup()
...
def test_hf_invalid_column_names_with_read_hf():
    ...
    with unittest.mock.patch('datachain.lib.dc.hf.read_values') as mock_read_values:
        ...
        assert 'factual?' in output_schema or 'factual_' in output_schema  # May be normalized
        assert 'user-name' in output_schema or 'user_name' in output_schema  # May be normalized
        assert '123column' in output_schema or 'c0_123column' in output_schema  # May be normalized
