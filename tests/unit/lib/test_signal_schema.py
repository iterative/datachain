...
def test_to_partial_complex_signal_mixed_entire_and_fields():
    ...
    assert serialized["file2"].startswith("FilePartial") and serialized["file2"].endswith("@v1")
...
def test_to_partial_complex_signal_error_invalid_signal():
    ...
    with pytest.raises(SignalSchemaError, match="Column nonexistent not found in the schema"):
...
def test_to_partial_complex_signal_error_invalid_field():
    ...
    with pytest.raises(SignalSchemaError, match="Field nonexistent not found in custom type"):
