...
def test_full_outer_join_no_common_rows(cloud_test_catalog, dogs_dataset, cats_dataset):
    catalog = cloud_test_catalog.catalog
    ...
    res = dogs.join(
        cats,
        dogs.c("file__path") == cats.c("file__path"),
        full=True,
    ).to_db_records()

    expected = [
        (string_default, "cats/cat1"),
        (string_default, "cats/cat2"),
        ("dogs/dog1", string_default),
        ("dogs/dog2", string_default),
        ("dogs/dog3", string_default),
        ("dogs/others/dog4", string_default),
    ]

    assert (
        sorted(
            ((r["file__path"], r["file__path_right"]) for r in res),
...
def test_full_outer_join_different_schemas(
    cloud_test_catalog, dogs_cats_dataset, cats_dataset
):
    catalog = cloud_test_catalog.catalog
    string_default = String.default_value(catalog.warehouse.db.dialect)

    DatasetQuery(cats_dataset.name, catalog=catalog).select(
        C("file.source"), C("file.path")
    ).save("cats_small", version=1)
    dogs_cats = DatasetQuery(name=dogs_cats_dataset.name, version=1, catalog=catalog)
    cats = DatasetQuery(name="cats_small", version=1, catalog=catalog)
