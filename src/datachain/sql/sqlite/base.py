...
def adapters_registered() -> bool:
    """
    Returns True if DataChain SQLite adapters and converters are registered.

    This function checks for the presence of key adapters and converters that
    DataChain registers with sqlite3, including:
    - Type adapters for list, dict, and numpy types
    - Type converters for DATETIME and ARRAY
    - DataChain's custom datetime adapter (replaces Python's default)

    Returns:
        bool: True if DataChain adapters are detected, False otherwise.

    Raises:
        AssertionError: If any required adapters or converters are missing,
                       with detailed information about what's missing.
    """
    missing_items = []

    # Check for key converters that DataChain registers
    datachain_converters = {"DATETIME", "ARRAY"}
    missing_converters = []
    for converter in datachain_converters:
        if converter not in sqlite3.converters:
            missing_converters.append(converter)

    if missing_converters:
        missing_items.append(f"Missing converters: {missing_converters}")

    # Check for key adapters that DataChain registers
    datachain_adapter_types = [list, dict]
    missing_adapters = []
    for adapter_type in datachain_adapter_types:
        adapter_key = (adapter_type, sqlite3.PrepareProtocol)
        if adapter_key not in sqlite3.adapters:
            missing_adapters.append(adapter_type.__name__)

    if missing_adapters:
        missing_items.append(f"Missing adapters: {missing_adapters}")

    # Check if DataChain's datetime adapter is registered (replaces Python's default)
    datetime_key = (datetime, sqlite3.PrepareProtocol)
    if datetime_key in sqlite3.adapters:
        adapter_func = sqlite3.adapters[datetime_key]
        # Check if it's DataChain's adapter by checking the module
        if not (
            hasattr(adapter_func, "__module__")
            and "datachain" in adapter_func.__module__
        ):
            module_name = (
                adapter_func.__module__
                if hasattr(adapter_func, "__module__")
                else "unknown"
            )
            missing_items.append(
                f"DateTime adapter is not DataChain's version (found: {module_name})"
            )
    else:
        missing_items.append("Missing datetime adapter")

    # If anything is missing, raise detailed error
    if missing_items:
        error_msg = "DataChain SQLite adapters not properly registered:\n" + "\n".join(
